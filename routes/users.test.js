...
const { commonBeforeAll, commonBeforeEach, commonAfterEach, commonAfterAll, u1Token, adminToken, u2Token, testJobs } = require("./_testCommon");
...
describe("POST /users/:username/jobs/:id", () => {
  test("works for admin", async () => {
    const resp = await request(app).post(`/users/u1/jobs/${testJobs[0]}`).set("authorization", `Bearer ${adminToken}`);

    expect(resp.body).toEqual({ applied: testJobs[0] });
  });

  test("works for logged-in user", async () => {
    const resp = await request(app).post(`/users/u1/jobs/${testJobs[0]}`).set("authorization", `Bearer ${u1Token}`);

    expect(resp.body).toEqual({ applied: testJobs[0] });
  });

  test("unauth for non logged-in user and non admin", async () => {
    const resp = await request(app).post(`/users/u1/jobs/${testJobs[0]}`).set("authorization", `Bearer ${u2Token}`);

    expect(resp.statusCode).toEqual(401);
  });

  test("unauth for anon", async () => {
    const resp = await request(app).post(`/users/u1/jobs/${testJobs[0]}`);

    expect(resp.statusCode).toEqual(401);
  });

  test("not found for no such user", async () => {
    const resp = await request(app).post(`/users/noUser/jobs/${testJobs[0]}`).set("authorization", `Bearer ${adminToken}`);
