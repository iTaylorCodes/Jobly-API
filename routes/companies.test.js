...
const { commonBeforeAll, commonBeforeEach, commonAfterEach, commonAfterAll, u1Token, adminToken, testJobs } = require("./_testCommon");
...
describe("GET /companies", function () {
  ...
  test("works for anon: uses 'name' filter", async () => {
  ...
  test("works for anon: uses 'minEmployees' and 'maxEmployees' filter", async () => {
  ...
  test("works for anon: bad request for minEmployees greater than maxEmployees", async () => {
    const resp = await request(app).get("/companies?minEmployees=3&maxEmployees=2");

    expect(resp.statusCode).toEqual(400);
  });

  test("works for anon: bad request for invalid filter key", async () => {
    const resp = await request(app).get("/companies?name=son&fakeKey=fake");
